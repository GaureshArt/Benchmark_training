            ...
            <ul className="flex justify-start items-center flex-col gap-4 p-2 text-slate-100">
              ...
              {state.map((task) => {
                return (
                  <li
                    key={task.id}
                    className="border border-slate-200 rounded-md w-4/5 h-15 flex gap-5 pr-4  pl-5 items-center"
                  >
                    <span className="w-full text-lg">Task: {task.task}</span>
                    <span
                      className={`w-20 h-8 rounded-lg border p-2 text-center flex justify-center items-center ${
                    ...
                    <button
                      className="w-20 h-10 border rounded  bg-red-500 border-red-300 p-2 cursor-pointer"
