...
export const Tasks = ()=>{
   ...
   const  handleRemoveTask = useCallback((task:TaskType)=>{
    console.log('Remove Task')
    dispatch({type:'REMOVE_TASK',info:{...task}});
    },[dispatch])
    const handleStatus = useCallback((task:TaskType)=>{
    ...
    return (
        ...
        <button className="w-25 h-10 border text-slate-100 border-slate-200 rounded" onClick={()=>setCount(count+1)}>Increment</button>
        <TaskInput handleAddTask = {handleAddTask} />
        <TaskList  handleRemoveTask = {handleRemoveTask} handleStatus = {handleStatus} totalCompletedTask = {totalCompletedTask} state={state}
/>
        </div>
        </>
    )
