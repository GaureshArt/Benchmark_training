...
const createCartCard = (
  ...
  cartCard.innerHTML = `
        ...
        <div class="product-rating">⭐ ${prodData.rating.rate} (${
    prodData.rating.count
  } reviews) <button class="edit-cart">Edit</button></div>
        
        <div class="product-price"> Base Price:$${prodData.price}
        <button class="remove-cart">Remove</button>
        </div>
        <div class="total-price-quantity">
            <div class="total-price-div">Total Price: $<span class="total-price">${
              quantity * prodData.price
            }</span></div>
...
const handleSaveCart = (cartCard: HTMLDivElement, editDiv: HTMLDivElement) => {
  ...
  const quantityDiv = cartCard.querySelector(".quantity") as HTMLDivElement;
  const quantityValue = (
    quantityDiv.querySelector(".quantity-value") as HTMLDivElement
  ).innerText;
  quantityDiv.innerHTML = `
...
mainSection.addEventListener("click", (e: Event): void => {
  ...
  if (
    targetElement.classList.contains("edit-cart") &&
...
const handleQuantityAndPriceValue = (val: number, cartCard: HTMLDivElement) => {
  ...
  const productTotalPriceDiv = cartCard.querySelector(
    ".total-price"
  ) as HTMLDivElement;
  const quantityValueDiv = cartCard.querySelector(
    ".quantity-value"
  ) as HTMLDivElement;
